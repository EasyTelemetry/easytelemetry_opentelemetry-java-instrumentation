...
public class Instrumenter<REQUEST, RESPONSE> {
  ...
  @Nullable
  RequestDataExtractor<REQUEST> requestDataExtractor;
  ...
  public Instrumenter setRequestDataExtractor(RequestDataExtractor<REQUEST> requestDataExtractor) {
  ...
  private void doEnd(
    ...
    if (requestDataExtractor != null && span.getSpanContext().isSampled()) {
      requestDataExtractor.doDataExtracting(request, span, context);
    }
